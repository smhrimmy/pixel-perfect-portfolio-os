...
import {
  BookOpen, Feather, Bookmark, Scroll, Sparkles, X, ArrowUpRight, CheckCircle2, Send, Sliders, Layers, Compass, Activity, Radio
} from "lucide-react";
...
function playAudio(type: 'radar' | 'chime' | 'pulse' | 'click' | 'warp', isMuted: boolean) {
  ...
  try {
    ...
    if (type === 'radar') {
      ...
      osc.frequency.exponentialRampToValueAtTime(1640, now + 0.2);
...
export default function EditorialSerifTheme({ data }: ThemeRendererProps) {
  const profile = (data as any)?.profile || (data as any)?.identity || {};
  const candidateName = profile?.name || "Prajwal DL";
  const bio = profile?.bio || "Private mahogany library reading room with pull-out vellum book folios and wax-sealed letters.";
  ...
  // 3D Procedural Heightfield Canvas Engine for The Reading Room
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
          ...
          const elevation = (wave1 + wave2 + ripple + mouseWarp) * 1.1;
  ...
  return (
    <div className="min-h-screen bg-[#120907] text-[#FFEDD5] font-serif relative overflow-x-hidden selection:bg-[#FB923C] selection:text-black">
      <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none z-0" />
      <div className="fixed inset-0 pointer-events-none z-10 bg-[radial-gradient(ellipse_at_center,transparent_0%,rgba(18,9,7,0.85)_80%)]" />

      {/* TOP HUD */}
      <header className="fixed top-0 inset-x-0 z-40 flex justify-between items-center px-6 py-4 bg-[#1F100D]/90 border-b border-[#FB923C]/35 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#FB923C]/15 border border-[#FB923C] text-[#FB923C] flex items-center justify-center shadow-[0_0_15px_rgba(251,146,60,0.4)]">
            <BookOpen className="w-5 h-5" />
          </div>
          <div>
            <h1 className="text-xs sm:text-sm font-bold tracking-widest uppercase flex items-center gap-2 text-[#FFF7ED]">
              <span>{candidateName}</span>
              <span className="text-[10px] px-2 py-0.5 rounded bg-[#FB923C]/15 text-[#FB923C] border border-[#FB923C]/40 font-mono">
                HIGGSFIELD AI MCF
              </span>
            </h1>
        ...
        <button
          ...
          className="w-9 h-9 rounded-xl bg-[#2E1813] border border-[#FB923C]/30 text-[#FB923C] flex items-center justify-center hover:bg-[#FB923C] hover:text-black transition cursor-pointer"
        >
      ...
      <main className="relative z-20 pt-32 pb-24 px-6 max-w-5xl mx-auto space-y-20">
        ...
        <section className="text-center space-y-6 pt-6">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#FB923C]/15 border border-[#FB923C]/40 text-[#FB923C] text-xs font-mono"
          >
            <BookOpen className="w-3.5 h-3.5" /> MAHOGANY READING ROOM · HIGGSFIELD MCF
          </motion.div>
          ...
          <motion.h2
            ...
            className="text-4xl sm:text-7xl font-bold tracking-tight uppercase text-[#FFF7ED] drop-shadow-[0_2px_30px_rgba(251,146,60,0.4)]"
          >
            Editorial <span class="text-[#FB923C] italic">Folio</span>
          </motion.h2>
        ...
        <section className="space-y-8">
          <div className="flex items-center justify-between border-b border-[#FB923C]/30 pb-4">
            <h3 className="text-xl font-bold text-[#FFF7ED] flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-[#FB923C]" /> Featured Projects & Systems
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((item) => (
              <motion.div
                key={item.id}
                whileHover={{ y: -4, borderColor: "#FB923C" }}
                onClick={() => {
                  setSelectedNode(item);
                  playAudio('radar', isMuted);
                }}
                className="p-6 rounded-2xl bg-[#1F100D]/90 border border-[#FB923C]/25 backdrop-blur-md cursor-pointer transition-all duration-300 shadow-[0_4px_25px_rgba(0,0,0,0.7)] group relative overflow-hidden"
              >
                <div className="flex justify-between items-center text-[10px] text-[#FB923C] font-mono mb-3">
                  <span className="px-2 py-0.5 rounded bg-[#FB923C]/15 border border-[#FB923C]/40">{item.badge}</span>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4 font-mono">
                  {item.tech.map((t) => (
                    <span key={t} className="text-[10px] px-2 py-0.5 rounded bg-[#120907] text-[#FB923C] border border-[#FB923C]/20">
                      {t}
                    </span>
                  ))}
                </div>
        ...
        <section className="space-y-6">
          <div className="border-b border-[#FB923C]/30 pb-4">
            <h3 className="text-xl font-bold text-[#FFF7ED] flex items-center gap-2">
          ...
          <div className="space-y-4">
            {careerTimeline.map((item, i) => (
              <div key={i} className="p-5 rounded-2xl bg-[#1F100D]/90 border border-[#FB923C]/25 flex flex-col sm:flex-row sm:items-center justify-between gap-4 backdrop-blur-sm">
                <div className="space-y-1">
                  <span className="text-[10px] px-2 py-0.5 rounded bg-[#FB923C]/15 text-[#FB923C] font-mono border border-[#FB923C]/40">
                    {item.period}
                  </span>
        ...
        {/* CONTACT DISPATCH */}
        <section className="p-8 rounded-3xl bg-[#1F100D]/90 border border-[#FB923C]/40 shadow-[0_0_40px_rgba(251,146,60,0.4)] space-y-6">
          ...
          {formSent ? (
            <div className="p-6 rounded-2xl bg-[#FB923C]/15 border border-[#FB923C]/40 text-center space-y-2">
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input required defaultValue="System Engineer" className="w-full px-4 py-2.5 rounded-xl bg-[#120907] border border-[#FB923C]/30 text-[#FFF7ED] focus:outline-none focus:border-[#FB923C]" />
                </div>
                <div>
                  <label className="block text-[#FB923C] font-mono mb-1">CORRESPONDENCE EMAIL</label>
                  <input required type="email" defaultValue="operator@telemetry.space" className="w-full px-4 py-2.5 rounded-xl bg-[#120907] border border-[#FB923C]/30 text-[#FFF7ED] focus:outline-none focus:border-[#FB923C]" />
                </div>
              </div>
              <div>
                <label className="block text-[#FB923C] font-mono mb-1">DISPATCH INQUIRY</label>
                <textarea rows={3} required defaultValue="Requesting full-stack architecture design with real-time 3D WebGL interfaces." className="w-full px-4 py-2.5 rounded-xl bg-[#120907] border border-[#FB923C]/30 text-[#FFF7ED] focus:outline-none focus:border-[#FB923C]" />
              </div>
              <button type="submit" className="w-full py-3 rounded-xl bg-[#FB923C] text-black font-mono font-bold text-xs hover:bg-[#FDBA74] transition flex items-center justify-center gap-2 cursor-pointer shadow-[0_0_20px_rgba(251,146,60,0.4)]">
                <Send className="w-4 h-4" /> TRANSMIT DISPATCH
              </button>
            </form>
          )}

          <div className="pt-4 border-t border-[#FB923C]/30 flex flex-wrap justify-between items-center text-[11px] text-slate-400 font-mono">
      ...
      <AnimatePresence>
        {selectedNode && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1F100D] border-2 border-[#FB923C] rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-[0_0_50px_rgba(251,146,60,0.4)] relative space-y-6">
              <button onClick={() => { setSelectedNode(null); playAudio('click', isMuted); }} className="absolute top-5 right-5 w-8 h-8 rounded-full bg-[#FB923C]/15 text-[#FB923C] hover:bg-[#FB923C] hover:text-black flex items-center justify-center transition cursor-pointer">
                <X className="w-4 h-4" />
              </button>
              <div className="space-y-1 font-mono">
                <span className="text-[10px] px-2 py-0.5 rounded bg-[#FB923C]/15 text-[#FB923C] border border-[#FB923C]/40">{selectedNode.badge}</span>
              ...
              <div className="p-3.5 rounded-xl bg-[#120907] border border-[#FB923C]/20 text-xs text-[#FB923C] font-mono">★ HIGHLIGHT: {selectedNode.highlight}</div>
              <div className="space-y-2 font-mono">
                <span className="text-xs text-slate-400">TECH TOKENS</span>
                <div className="flex flex-wrap gap-2">
                  {selectedNode.tech.map((t: string) => (
                    <span key={t} className="text-xs px-2.5 py-1 rounded-lg bg-[#120907] text-[#FFF7ED] border border-[#FB923C]/20">{t}</span>
                  ))}
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <a href={selectedNode.liveUrl} target="_blank" rel="noreferrer" className="flex-1 py-2.5 rounded-xl bg-[#FB923C] text-black font-bold font-mono text-xs text-center hover:bg-[#FDBA74] transition flex items-center justify-center gap-1.5">
