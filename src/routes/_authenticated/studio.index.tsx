...
function StudioDashboard() {
  ...
  const seoAudit = runSEOAudit({
    title: settings?.siteTitle || "Prajwal DL — Portfolio OS",
    description: settings?.siteDescription || "Full Stack Developer & Web Advisor",
  ...
  const quickActions = [
    ...
    { label: "Theme HQ", href: "/studio/hq-terminal", icon: Palette, color: "text-indigo-400 bg-indigo-500/10 border-indigo-500/30" },
    { label: "AI Copilot", href: "/studio/ai", icon: Bot, color: "text-fuchsia-400 bg-fuchsia-500/10 border-fuchsia-500/30" },
    { label: "SEO Audit", href: "/studio/seo", icon: Search, color: "text-yellow-400 bg-yellow-500/10 border-yellow-500/30" },
  ];
  ...
  return (
    <div className="min-h-full space-y-8 pb-12 font-sans">
      ...
      {/* ==================================================== */}
      {/* 1. 4D SPATIAL TELEMETRY & GREETING HEADER           */}
      {/* ==================================================== */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1E2630]/60 pb-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-mono font-bold tracking-widest text-[#00F5D4] uppercase">
              4D SPATIAL COMMAND CENTER · ACTIVE
            </span>
          </div>
          ...
          <p className="text-xs sm:text-sm text-[#9AA6B2] mt-1">
            Portfolio OS is fully synchronized. Live theme active across all dimensions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <Button
            ...
            variant="outline"
            size="sm"
            className="border-[#1E2630] bg-[#11161D] text-[#E6F1FF] hover:bg-[#1E2630] text-xs h-9"
          >
            <FileDown className="h-3.5 w-3.5 mr-1.5 text-amber-400" /> Export PDF
          </Button>
          ...
          <Button asChild size="sm" className="bg-gradient-to-r from-[#00E6C3] to-teal-500 text-black hover:opacity-95 font-bold text-xs h-9 shadow-lg shadow-[#00E6C3]/20">
            <Link to="/studio/hq-terminal">
              <Upload className="h-3.5 w-3.5 mr-1.5" /> Publish Live
            </Link>
          </Button>
        </div>
      </div>
      ...
      {/* ==================================================== */}
      {/* 2. HERO ROW: 4D LIVE PREVIEW + QUICK ACTIONS         */}
      {/* ==================================================== */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 w-full min-w-0">
        {/* Left: 4D Live Website Preview Card */}
        <div className="xl:col-span-6 rounded-3xl border border-[#1E2630] bg-[#11161D] p-5 flex flex-col justify-between relative overflow-hidden group shadow-xl">
          ...
          <div className="my-5 rounded-2xl border border-[#1E2630] bg-gradient-to-br from-[#07090D] via-[#0B0F14] to-[#120B20] p-6 text-center relative overflow-hidden flex flex-col items-center justify-center min-h-[160px]">
            ...
            <div className="text-[10px] font-mono text-[#00E6C3] uppercase tracking-[0.3em] font-semibold">// Dimension α</div>
            <h2 className="text-2xl font-black tracking-tight text-white mt-1">PRAJWAL DL</h2>
            <p className="text-xs text-[#9AA6B2] mt-1 font-medium">Full Stack Developer &amp; Web Advisor</p>
            <div className="flex items-center gap-3 mt-4 text-[#9AA6B2] text-xs">
              <span className="h-1.5 w-1.5 rounded-full bg-[#00E6C3]" />
              <span className="font-mono text-[10px]">21 Real-World Physical &amp; 4D Themes</span>
            </div>
          </div>

          <div className="flex items-center justify-between z-10 pt-2 border-t border-[#1E2630]/60">
            <span className="text-[11px] text-[#9AA6B2] font-mono">Status: Live &amp; Resilient Memory Cache</span>
            <Button asChild size="sm" variant="ghost" className="text-xs text-[#00E6C3] hover:text-white h-7">
              <Link to="/studio/hq-terminal">
                Switch Theme <ChevronRight className="h-3 w-3 ml-1" />
              </Link>
            </Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 w-full min-w-0">
              ...
              {analytics.topProjects.map((p) => (
                <div key={p.id} className="p-3 rounded-2xl border border-[#1E2630] bg-[#0B0F14] space-y-1.5">
                  ...
                  <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-[#00F5D4] to-teal-400 rounded-full"
        ...
        <div className="rounded-3xl border border-[#1E2630] bg-[#11161D] p-5 flex flex-col justify-between shadow-xl">
          <div>
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-mono font-bold uppercase tracking-wider text-[#9AA6B2]">SEO Audit Health</span>
            ...
            <div className="space-y-2 text-xs">
              {seoAudit.checks.map((c) => (
                <div key={c.id} className="flex items-center justify-between p-1.5 rounded-lg hover:bg-white/5 transition">
                  ...
                  <span className="text-[10px] font-mono text-[#9AA6B2]">{c.status === "good" ? "Passed" : "Warning"}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="pt-4 mt-4 border-t border-[#1E2630]">
            <Link to="/studio/seo" className="text-xs text-[#00E6C3] hover:underline flex items-center gap-1 font-semibold">
